...
class ModelBuilder:
    ...
    def create_teacher_model(input_shape=(*Config.IMAGE_SIZE, Config.CHANNELS), num_classes=Config.NUM_CLASSES):
        ...
        def dynamic_feature_fusion(spatial_feat, freq_feat, temporal_feat):
            ...
            quality = quality_net(tf.concat([spatial, freq, temporal], axis=-1))
            
            enhanced_spatial = quality[:,0:1] * enhance_net(spatial)
            enhanced_freq = quality[:,1:2] * enhance_net(freq)
            enhanced_temporal = quality[:,2:3] * enhance_net(temporal)

            fused = enhanced_spatial + enhanced_freq + enhanced_temporal
            fused += layers.Dense(256)(tf.concat([spatial, freq, temporal], axis=-1))
        ...
        fused_feat = dynamic_feature_fusion(spatial_feat, freq_feat, temporal_feat)
    ...
    def create_student_model(input_shape=(*Config.IMAGE_SIZE, Config.CHANNELS), num_classes=Config.NUM_CLASSES):
        ...
        x_spatial = layers.Conv2D(128, (1,1), padding='same', activation='relu', name='student_spatial_adapter')(x)
        x_spatial = layers.BatchNormalization()(x_spatial)

        x_reshape = layers.Reshape((-1, 128))(x)
        vit = ModelBuilder.create_vit1d(input_shape=(25, 128), num_heads=2, embed_dim=32, num_transformer_blocks=1)
        ...
        def student_fusion(spatial, temporal, freq):
            ...
            gate_net = models.Sequential([
                ...
                layers.Dense(3, activation='softmax')])
            gates = gate_net(tf.concat([spatial, temporal, freq], axis=-1))
            fused = gates[:,0:1]*spatial + gates[:,1:2]*temporal + gates[:,2:3]*freq
            return fused
        
        fused_features = student_fusion(x_spatial, x_temporal, x_freq)
...
class Trainer:
    ...
    def train_student(self, train_ds, val_ds):
        ...
        for epoch in range(Config.EPOCHS):
            current_weights = tf.nn.softmax(loss_fn.domain_weights).numpy()
            self.theta_history.append({
                'epoch': epoch + 1,
                'spatial': current_weights[0],
                'temporal': current_weights[1],
                'frequency': current_weights[2],
                'fused': current_weights[3]})
            ...
            epoch_losses = {k: [] for k in [
                ...
                'spatial', 'temporal', 'frequency', 'fused',
                'weight_spatial', 'weight_temporal', 'weight_frequency', 'weight_fused']}
            ...
            self.train_acc_history.append(train_acc.result().numpy())
            self.val_acc_history.append(val_acc.result().numpy())
        ...
        self._plot_theta_curve_from_memory()
    ...
    def _print_epoch_summary(self, epoch, losses, train_acc, val_acc):
        ...
        print(f"Total Loss: {np.mean(losses['total']):.4f} | "
              ...
              f"spatial_feature_loss: {np.mean(losses['spatial']):.4f}\n"
              f"freq_feature_loss: {np.mean(losses['frequency']):.4f} | "
              f"fused_feature_loss: {np.mean(losses['fused']):.4f}")
        print(f"Weight allocation - spatial: {np.mean(losses['weight_spatial']):.3f} | "
              f"temporal: {np.mean(losses['weight_temporal']):.3f} | "
    ...
    def _plot_theta_curve_from_memory(self):
        """直接从内存记录的数据绘制权重曲线"""
        if not self.theta_history:
            print("警告：没有权重记录数据，跳过绘图")
            return
        
        df = pd.DataFrame(self.theta_history)
        
        plt.figure(figsize=(10, 6))
        plt.plot(df['epoch'], df['spatial'], label='Spatial Weight', marker='o')
        plt.plot(df['epoch'], df['temporal'], label='Temporal Weight', marker='s')
        plt.plot(df['epoch'], df['frequency'], label='Frequency Weight', marker='^')
        plt.plot(df['epoch'], df['fused'], label='Fused Weight', marker='d')
        
        plt.xlabel('Epoch')
        plt.ylabel('Weight Value')
        plt.title('Dynamic Domain Weights During Training')
        plt.legend()
        plt.grid(True)
        
        os.makedirs(Config.EVAL_PLOT_PATH, exist_ok=True)
        plot_path = os.path.join(Config.EVAL_PLOT_PATH, 'theta_learning_curve.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"权重曲线已保存至: {plot_path}")

    def _plot_accuracy_curve(self):
        """绘制训练和验证精度变化曲线"""
        if not self.train_acc_history or not self.val_acc_history:
            print("警告：没有精度记录数据，跳过绘图")
            return
        
        plt.figure(figsize=(10, 6))
        epochs = range(1, len(self.train_acc_history) + 1)
        
        plt.plot(epochs, self.train_acc_history, 'b-', label='Training Accuracy')
        ...
        # 保存图像
        os.makedirs(Config.EVAL_PLOT_PATH, exist_ok=True)
        plot_path = os.path.join(Config.EVAL_PLOT_PATH, 'accuracy_curve.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"精度曲线已保存至: {plot_path}")
        # ========================================
# ------------------------------
...
class AdaptiveDistillationLoss(tf.keras.losses.Loss):
    def __init__(self, teacher_model, temperature=5.0, alpha=0.5, name='adaptive_distill_loss'):
        ...
        self.domain_weights = tf.Variable(initial_value=[1.0, 1.0, 1.0, 1.0], trainable=True, dtype=tf.float32, name='domain_weights')
        
        self.kl_loss = tf.keras.losses.KLDivergence()
        self.cce_loss = tf.keras.losses.CategoricalCrossentropy()
        
        self._create_aligners(teacher_model)
    
    def _create_aligners(self, teacher_model):
        dummy_input = tf.random.normal([1, *Config.IMAGE_SIZE, Config.CHANNELS])
        teacher_outputs = teacher_model(dummy_input)
        
        spatial_shape = teacher_outputs[1].shape[1:]
        self.spatial_aligner = ModelBuilder.create_spindle_mlp(
            input_dim=int(tf.reduce_prod(spatial_shape)),
            output_dim=int(tf.reduce_prod(spatial_shape)),
            name='spatial_aligner')
        
        temporal_dim = teacher_outputs[2].shape[-1]
        self.temporal_aligner = ModelBuilder.create_spindle_mlp(
            input_dim=temporal_dim,
            output_dim=temporal_dim,
            name='temporal_aligner')
        
        freq_dim = teacher_outputs[3].shape[-1]
        self.frequency_aligner = ModelBuilder.create_spindle_mlp(
            input_dim=freq_dim,
            output_dim=freq_dim,
            name='frequency_aligner')
        
        fused_dim = teacher_outputs[4].shape[-1]
    ...
    def call(self, y_true, y_pred):
        ...
        aligned_student = {
            'spatial': self.spatial_aligner(layers.Flatten()(y_pred['spatial'])),
        ...
        aligned_teacher = {
            'spatial': self.spatial_aligner(layers.Flatten()(teacher_outputs[1])),
            'temporal': self.temporal_aligner(teacher_outputs[2]),
            'frequency': self.frequency_aligner(teacher_outputs[3]),
            'fused': self.fused_aligner(teacher_outputs[4])}
        ...
        feature_losses = {
            'spatial': tf.reduce_mean(tf.square(aligned_teacher['spatial'] - aligned_student['spatial'])),
        ...
        weighted_feature_loss = (
            weights[0] * feature_losses['spatial'] +
            weights[1] * feature_losses['temporal'] +
            weights[2] * feature_losses['frequency'] +
            weights[3] * feature_losses['fused'])
        
        total_loss = (self.alpha * classification_loss + (1 - self.alpha) * distillation_loss + weighted_feature_loss)
        
        self.last_losses = {
            'total': total_loss,
            'classification': classification_loss,
            'distillation': distillation_loss,
            'spatial': feature_losses['spatial'],
            'temporal': feature_losses['temporal'],
            'frequency': feature_losses['frequency'],
            'fused': feature_losses['fused'],
            'weight_spatial': weights[0],
            'weight_temporal': weights[1],
            'weight_frequency': weights[2],
            'weight_fused': weights[3]}
        return total_loss
