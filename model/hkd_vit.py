...
class ModelBuilder:
    ...
    def create_teacher_model(input_shape=(*Config.IMAGE_SIZE, Config.CHANNELS), num_classes=Config.NUM_CLASSES):
        ...
        def spatial_feature_extractor():
            ...
            x = ModelBuilder.residual_block(inp, 32, prefix='spatial_block1')
            x = ModelBuilder.residual_block(x, 64, prefix='spatial_block2')
            x = ModelBuilder.residual_block(x, 128, prefix='spatial_block3')
            return models.Model(inputs=inp, outputs=x, name='spatial_extractor')

        def frequency_feature_extractor():
            inp = layers.Input(shape=input_shape)
            x = ModelBuilder.wavelet_transform_block(inp, wavelet_name='db1', mode='symmetric')
            x = layers.Conv2D(256, (3, 3), padding='same', activation='relu')(x)
            x = layers.BatchNormalization()(x)
            x = layers.GlobalAveragePooling2D()(x)
            x = layers.Dense(256, activation='relu')(x)
            return models.Model(inputs=inp, outputs=x, name='frequency_extractor')

        def dynamic_feature_fusion(spatial_feat, freq_feat):
            ...
            quality = quality_net(tf.concat([spatial, freq], axis=-1))
            enhanced_spatial = quality[:, 0:1] * enhance_net(spatial)
            enhanced_freq   = quality[:, 1:2] * enhance_net(freq)

            fused = enhanced_spatial + enhanced_freq
            fused += layers.Dense(256)(tf.concat([spatial, freq], axis=-1))
        ...
        fused_feat   = dynamic_feature_fusion(spatial_feat, freq_feat)

        x = layers.Dense(256, activation='relu')(fused_feat)
        x = layers.BatchNormalization()(x)
        ...
        return models.Model(
            inputs=inputs,
            outputs=[output_main, output_spatial, output_frequency, output_fused],
            name='teacher_model')
    ...
    def create_student_model(input_shape=(*Config.IMAGE_SIZE, Config.CHANNELS), num_classes=Config.NUM_CLASSES):
        ...
        def student_fusion(spatial, freq):
            spatial_flat = layers.GlobalAveragePooling2D()(spatial)
            combined = layers.Concatenate()([spatial_flat, freq])
            gate_net = models.Sequential([
                layers.Dense(64, activation='relu'),
                layers.Dense(2, activation='softmax')
            ], name='student_gate')
            gates = gate_net(combined)
            spatial = layers.Dense(256)(spatial_flat)
            freq = layers.Dense(256)(freq)
            fused = (gates[:, 0:1] * spatial + gates[:, 1:2] * freq)
            return fused

        fused_features = student_fusion(x_spatial, x_freq)
    ...
    def train_student(self, train_ds, val_ds):
        ...
        for epoch in range(Config.EPOCHS):
            ...
            epoch_losses = {k: [] for k in [
                ...
                'weight_spatial', 'weight_frequency', 'weight_fused']}
