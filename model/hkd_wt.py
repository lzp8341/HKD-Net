...
class ModelBuilder:
    ...
    def wavelet_transform_block(x, wavelet_name='db1', mode='symmetric'):
        ...
        x_decomposed = layers.Lambda(
            ...
            name='shape_ensurer')(x_decomposed)
    ...
    def create_teacher_model(input_shape=(*Config.IMAGE_SIZE, Config.CHANNELS), num_classes=Config.NUM_CLASSES):
        ...
        x = ModelBuilder.residual_block(inputs, 32, prefix='spatial1')
        x = ModelBuilder.residual_block(x, 64, prefix='spatial2')
        x = ModelBuilder.residual_block(x, 128, prefix='spatial3')
        spatial_feat = layers.GlobalAveragePooling2D()(x)          # (None, 128)

        x_t = layers.Reshape((Config.IMAGE_SIZE[0] * Config.IMAGE_SIZE[1],))(inputs)
        x_t = layers.Reshape((Config.IMAGE_SIZE[0], Config.IMAGE_SIZE[1]))(x_t)
        temporal_feat = ModelBuilder.create_vit1d(
            (Config.IMAGE_SIZE[0], Config.IMAGE_SIZE[1]),
            embed_dim=Config.TEMPORAL_DIM,
            num_heads=4,
            ff_dim=128,
            num_transformer_blocks=2)(x_t)                               

        spatial = layers.Dense(256)(spatial_feat)
        ...
        quality = quality_net(tf.concat([spatial, temporal], axis=-1))
        enhanced_s = quality[:, 0:1] * enhance_net(spatial)
        enhanced_t = quality[:, 1:2] * enhance_net(temporal)
        fused = enhanced_s + enhanced_t + layers.Dense(256)(tf.concat([spatial, temporal], axis=-1))
        fused = layers.LayerNormalization()(fused)

        x = layers.Dense(128, activation='relu')(fused)
        x = layers.Dropout(0.3)(x)
        logits = layers.Dense(num_classes, activation='softmax', name='classification')(x)

        return models.Model(inputs=inputs,
                            outputs=[logits, spatial_feat, temporal_feat, fused],
                            name='teacher_model')
    ...
    def create_student_model(input_shape=(*Config.IMAGE_SIZE, Config.CHANNELS),
        ...
        x = dw_block(inputs, 32, 0.15)
        x = dw_block(x, 64, 0.2)
        x = dw_block(x, 128, 0.25)              
        spatial_feat = layers.GlobalAveragePooling2D()(x)   
        x_t = layers.Reshape((40, 40))(inputs)
        temporal_feat = ModelBuilder.create_vit1d(
            (40, 40),
            embed_dim=32,
            num_heads=2,
            ff_dim=64,
            num_transformer_blocks=1)(x_t)
        temporal_feat = layers.Dense(Config.TEMPORAL_DIM, name='temporal_adapter')(temporal_feat)
        ...
        gate = models.Sequential([
            ...
            layers.Dense(2, activation='softmax')])
        g = gate(tf.concat([spatial, temporal], axis=-1))
        fused = g[:, 0:1] * spatial + g[:, 1:2] * temporal

        x = layers.Dense(128, activation='relu')(fused)
        x = layers.BatchNormalization()(x)
        x = layers.Dropout(0.4)(x)
        logits = layers.Dense(num_classes, activation='softmax', name='student_output')(x)
    ...
    def train_student(self, train_ds, val_ds):
        ...
        for epoch in range(Config.EPOCHS):
            ...
            epoch_losses = {k: [] for k in [
                ...
                'spatial', 'temporal', 'fused',
                'weight_spatial', 'weight_temporal', 'weight_fused']}
...
class AdaptiveDistillationLoss(tf.keras.losses.Loss):
    ...
    def build(self, input_shape):
        ...
        self.spatial_aligner.build((None, int(tf.reduce_prod(spatial_shape))))
    ...
    def call(self, y_true, y_pred):
        ...
        distillation_loss = self.kl_loss(
            ...
            tf.nn.softmax(y_pred['logits']/self.temperature, axis=1)) * (self.temperature ** 2)
    ...
    def trainable_variables(self):
        ...
        if self._built:
            vars.extend(self.spatial_aligner.trainable_variables)
